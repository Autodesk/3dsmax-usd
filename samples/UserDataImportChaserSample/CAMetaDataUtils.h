...
getOrDefineCABuiltInMetaData(const std::vector<std::pair<UsdMetaDataTypeCA, ParameterValue>>& cas);

static const UsdMetaDataDefCA getCAMetaDataDef(UsdMetaDataTypeCA type, const std::wstring& key)
{
    switch (type) {
    case CA_BOOL: return createCABoolMetaDataDef(key);
    case CA_INT: return createCAIntMetaDataDef(key);
    case CA_STR: return createCAStrMetaDataDef(key);
    default: return createCABoolMetaDataDef(L"not_found");
    }
}
