...
namespace Ui {
class UsdStageRenderSettingsRollup;
}

class IParamBlock2;

class UsdStageRenderSettingsRollup : public MaxSDK::QMaxParamBlockWidget
{
    Q_OBJECT

public:
    explicit UsdStageRenderSettingsRollup(ReferenceMaker& owner, IParamBlock2& paramBlock);
    virtual ~UsdStageRenderSettingsRollup();
...
public Q_SLOTS:
    ...
    void on_AssignMtlToNodeButton_clicked() const;

private:
    /// Model ParamBlock pointer
    IParamBlock2* paramBlock = nullptr;
    /// Reference to the Qt UI View of the rollup
    std::unique_ptr<Ui::UsdStageRenderSettingsRollup> ui {
        std::make_unique<Ui::UsdStageRenderSettingsRollup>()
    };
