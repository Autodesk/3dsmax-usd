...
#include <MaxUsdObjects/LayerEditor/USDLayerEditor.h>
                        ...
                        if (auto widget = map->GetQWidget()) {
                            if (auto rollup
                                ...
                                if (mapID != ParamMapID::UsdStageSelection) {
                                    ...
                                    constexpr auto kOriginalContentsMarginsKey
                                        = "__USDStageObject_Rollup_OriginalContentsMargins__";
                                    if (subObjectLevel == 0) {
                                        if (rollup->isHidden()) {
                                            rollup->setVisible(true);
                                            widget->setVisible(rollup->isOpen());
                                            QVariant v
                                                = rollup->property(kOriginalContentsMarginsKey);
                                            if (v.isValid() && v.canConvert<QVariantList>()) {
                                                ...
                                                if (vl.size() == 4) {
                                                    ...
                                                    bool     ok = false;
                                                    v = vl.at(0);
                                                    margins.setLeft(v.toInt(&ok));
                                                    if (ok) {
                                                        v = vl.at(1);
                                                        margins.setTop(v.toInt(&ok));
                                                    }
                                                    if (ok) {
                                                        v = vl.at(2);
                                                        margins.setRight(v.toInt(&ok));
                                                    }
                                                    if (ok) {
                                                        v = vl.at(3);
                                                        margins.setBottom(v.toInt(&ok));
                                                    }
                                                    if (ok) {
                                                        rollup->setContentsMargins(margins);
                                                    }
                                                }
                                                rollup->setProperty(
                                                    kOriginalContentsMarginsKey, {});
                                            }
                                        }
                                    } else if (rollup->isVisible()) {
                                        rollup->setVisible(false);
                                        widget->setVisible(false);
                                        QMargins     margins = rollup->contentsMargins();
                                        QVariantList marginsList = { margins.left(),
                                                                     margins.top(),
                                                                     margins.right(),
                                                                     margins.bottom() };
                                        rollup->setProperty(
                                            kOriginalContentsMarginsKey,
                                            QVariant::fromValue(marginsList));
...
void USDStageObject::OpenInUsdLayerEditor() { USDLayerEditor::Instance()->OpenStage(this); }
