...
TEST(CreaseDataConversionTests, SimpleCreaseDataConversion)
{
    ...
    converter.ConvertToUSDMesh(
        ...
        stage,
        path,
        MaxUsd::MaxMeshConversionOptions {},
        usdMesh,
    ...
    ASSERT_TRUE(
        creaseIndices.size() != 0 && creaseIndices.size() == 2 * creaseLengths.size()
...
TEST(CreaseDataConversionTests, TimeSampledCreases)
{
    ...
    auto setAtTimeCode = [&](const pxr::VtIntArray&   eIndices,
                             const pxr::VtIntArray&   eLengths,
                             const pxr::VtFloatArray& eSharpenesses,
                             const pxr::VtIntArray&   vIndices,
                             const pxr::VtFloatArray& vSharpnesses,
    ...
    setAtTimeCode(
        eIndicesDefault,
        eLengthsDefault,
        eSharpnesssesDefault,
        vIndicesDefault,
        vSharpnesssesDefault,
        pxr::UsdTimeCode::Default());
    ...
    auto testAtTimeCode = [&](const pxr::VtIntArray&   eIndices,
                              const pxr::VtIntArray&   eLengths,
                              const pxr::VtFloatArray& eSharpenesses,
                              const pxr::VtIntArray&   vIndices,
                              const pxr::VtFloatArray& vSharpnesses,
...
TEST(CreaseDataConversionTests, NoCreaseDataConversion)
{
    ...
    // Even if creases are enabled, if none are actually defined, we should not get the attributes
    // exported in USD.
    cube.setEDataSupport(EDATA_CREASE);
    cube.setVDataSupport(VDATA_CREASE);

    auto usdMeshWithCreaseSupport
        = pxr::UsdGeomMesh(stage->DefinePrim(path, pxr::TfToken("MeshWithCreaseSupport")));
    converter.ConvertToUSDMesh(
        MaxUsd::MeshFacade { &cube },
        stage,
        path,
    ...
    // Finally, test that crease support is not enabled on the Max side if there are no USD creases
    // authored.
    MNMesh                     reimportedMesh;
    std::map<int, std::string> channelNames;
    converter.ConvertToMNMesh(
