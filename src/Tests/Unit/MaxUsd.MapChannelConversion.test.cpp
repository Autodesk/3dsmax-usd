...
TEST(MapChannelConversionTests, MapChannelsConversion)
{
	...
	for (int i = -NUM_HIDDENMAPS; i < MAX_MESHMAPS; ++i)
	{
		...
		options.SetChannelPrimvarConfig(
				i, MaxUsd::MappedAttributeBuilder::Config(config.GetPrimvarName(), config.GetPrimvarType(), false));
	}
	...
	const std::vector<pxr::UsdGeomPrimvar> primvars = pxr::UsdGeomPrimvarsAPI(usdMesh).GetAuthoredPrimvars();
	...
	for (int i = -NUM_HIDDENMAPS; i < MAX_MESHMAPS; ++i)
	{
		...
		EXPECT_EQ(MaxUsd::MappedAttributeBuilder::GetValueTypeName(config.GetPrimvarType()), primvar.GetTypeName());
