...
TEST(ChannelBuilderTests, ConstantPrimvar)
{
	...
	auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("testPrimvar"), pxr::SdfValueTypeNames->Float3Array);
	...
	builder.Build(primvar.GetAttr(), primvar.GetInterpolation(), &primvar, usdMesh, pxr::UsdTimeCode::Default());
...
TEST(ChannelBuilderTests, FaceVaryingPrimvar)
{
	TestFaceVaryingPrimvar(false);
}

TEST(ChannelBuilderTests, FaceVaryingPrimvar_Indexed)
{
	TestFaceVaryingPrimvar(true);
}
...
void TestUniformPrimvar(bool indexed)
{
	...
	if (indexed)
	{
		...
		for (int i = 0; i < fNum; ++i)
		{
			...
			for (int j = 0; j < face->deg; ++j)
			{
				EXPECT_EQ(map->F(i)->tv[j], i > 0 ? 1 : 0); // First face uses value1, other faces value2.
			}
		}
	}
	else
...
TEST(ChannelBuilderTests, Uniform)
{
	TestUniformPrimvar(false);
}

TEST(ChannelBuilderTests, Uniform_Indexed)
{
	TestUniformPrimvar(true);
}
...
TEST(ChannelBuilderTests, TypeCasts)
{
	...
	// We should be able to cast a Double3Array to make it fit into a max channel (which carries floats).
	{
		auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("Double3ArrayPrimvar"), pxr::SdfValueTypeNames->Double3Array);
		primvar.SetInterpolation(pxr::UsdGeomTokens->constant);
	...
	{
		auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("Double2ArrayPrimvar"), pxr::SdfValueTypeNames->Double2Array);
		primvar.SetInterpolation(pxr::UsdGeomTokens->constant);
	...
	{
		auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("DoublePrimvar"), pxr::SdfValueTypeNames->DoubleArray);
		primvar.SetInterpolation(pxr::UsdGeomTokens->constant);
	...
	{
		auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("Double4ArrayPrimvar"), pxr::SdfValueTypeNames->Double4Array);
		primvar.SetInterpolation(pxr::UsdGeomTokens->constant);
	...
	// Double (non array type) should also work, casts to an array of doubles of size 1, which is accepted. And could be valid for a
	// constant primvar.
	{
		auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("DoublePrimvar"), pxr::SdfValueTypeNames->Double);
		primvar.SetInterpolation(pxr::UsdGeomTokens->constant);
	...
	{
		auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("StringPrimvar"), pxr::SdfValueTypeNames->String);
		primvar.SetInterpolation(pxr::UsdGeomTokens->constant);
		primvar.Set("foo");
		auto builder = MapBuilder(&cube, 0);
		builder.Build(primvar.GetAttr(), primvar.GetInterpolation(), &primvar, usdMesh, pxr::UsdTimeCode::Default());
...
TEST(ChannelBuilderTests, PrimvarChannelResolution)
{
	...
	auto primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar( floatArrayName, pxr::SdfValueTypeNames->FloatArray, pxr::UsdGeomTokens->constant);
	primvar.Set(floatArray);	
	primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar(float2Array1Name, pxr::SdfValueTypeNames->Float2Array, pxr::UsdGeomTokens->constant);
	primvar.Set(float2Array);	
	primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar(float2Array2Name, pxr::SdfValueTypeNames->Float2Array, pxr::UsdGeomTokens->constant);
	primvar.Set(float2Array);
	primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar(texCoord2fArrayName, pxr::SdfValueTypeNames->TexCoord2fArray, pxr::UsdGeomTokens->constant);
	primvar.Set(float2Array);
	primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar(texCoord3fArrayName, pxr::SdfValueTypeNames->TexCoord3fArray, pxr::UsdGeomTokens->constant);
	primvar.Set(float3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar(color3fArrayName, pxr::SdfValueTypeNames->Color3fArray, pxr::UsdGeomTokens->constant);
	primvar.Set(float3Array);
	...
	// Create a new primvar with a name higher by alphabetical order, type texcoord2fArray, it should now
	// be selected.
	auto aTexcoord2fArray = pxr::TfToken("aTexcoord2fArray");
	primvar = pxr::UsdGeomPrimvarsAPI(usdMesh).CreatePrimvar(aTexcoord2fArray, pxr::SdfValueTypeNames->TexCoord2fArray, pxr::UsdGeomTokens->constant);
	primvar.Set(float2Array);
	TestUtils::MeshConverterTester::ResolveChannelPrimvars(usdMesh, options, channelPrimvars);
	...
	EXPECT_EQ(channelPrimvars[2].GetPrimvarName(), texCoord2fArrayName); // Texcoord2 - from type order.
	EXPECT_EQ(channelPrimvars[3].GetPrimvarName(), texCoord3fArrayName); // TexCoord3 -from type order.
	EXPECT_EQ(channelPrimvars[4].GetPrimvarName(), float2Array1Name); // Next found primvars... order doesn't depend on type.
	...
	const auto color3dArray = pxr::TfToken("color3dArray"); // Will be inferred as vertex color (0) from type
	...
	const auto texCoord2fArray = pxr::TfToken("texCoord2fArray"); // -> Will be inferred as main UV (1) from type
	...
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(point3hArray, pxr::SdfValueTypeNames->Point3hArray, pxr::UsdGeomTokens->constant);
	primvar.Set(half3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(color3hArray, pxr::SdfValueTypeNames->Color3hArray, pxr::UsdGeomTokens->constant);
	primvar.Set(half3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(color3dArray, pxr::SdfValueTypeNames->Color3dArray, pxr::UsdGeomTokens->constant);
	primvar.Set(double3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(normal3dArray, pxr::SdfValueTypeNames->Normal3dArray, pxr::UsdGeomTokens->constant);
	primvar.Set(double3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(texCoord3hArray, pxr::SdfValueTypeNames->TexCoord3hArray, pxr::UsdGeomTokens->constant);
	primvar.Set(half3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(texCoord3dArray, pxr::SdfValueTypeNames->TexCoord3dArray, pxr::UsdGeomTokens->constant);
	primvar.Set(double3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(texCoord3fArray, pxr::SdfValueTypeNames->TexCoord3fArray, pxr::UsdGeomTokens->constant);
	primvar.Set(float3Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(texCoord2hArray, pxr::SdfValueTypeNames->TexCoord2hArray, pxr::UsdGeomTokens->constant);
	primvar.Set(half2Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(texCoord2dArray, pxr::SdfValueTypeNames->TexCoord2dArray, pxr::UsdGeomTokens->constant);
	primvar.Set(double2Array);
	primvar = pxr::UsdGeomPrimvarsAPI(orderMesh).CreatePrimvar(texCoord2fArray, pxr::SdfValueTypeNames->TexCoord2fArray, pxr::UsdGeomTokens->constant);
	primvar.Set(float2Array);
	...
	// Test that primvars which cannot be fit into max channels are ignored by the mapping resolution
	// unless the mapping is explicitly specified.
	const auto unusablePrimvars = pxr::SdfPath("/unusable_primvars");
	auto unusablePrimvarsMesh = pxr::UsdGeomMesh(stage->DefinePrim(unusablePrimvars, pxr::TfToken("Mesh")));
	// No values authored.
	primvar = pxr::UsdGeomPrimvarsAPI(unusablePrimvarsMesh).CreatePrimvar(point3hArray, pxr::SdfValueTypeNames->Point3hArray, pxr::UsdGeomTokens->constant);
	// String primvar
	primvar = pxr::UsdGeomPrimvarsAPI(unusablePrimvarsMesh).CreatePrimvar(pxr::TfToken("stringPrimvar"), pxr::SdfValueTypeNames->String, pxr::UsdGeomTokens->constant);
	primvar.Set("foo");
	// Dimension > 3
	const auto float4primvar = pxr::TfToken("float4primvar");
	primvar = pxr::UsdGeomPrimvarsAPI(unusablePrimvarsMesh).CreatePrimvar(float4primvar, pxr::SdfValueTypeNames->Float4Array, pxr::UsdGeomTokens->constant);
	primvar.Set(pxr::VtVec4fArray{ { 1.0f, 1.0f, 1.0f, 1.0f } });
	options.ClearMappedPrimvars();
	TestUtils::MeshConverterTester::ResolveChannelPrimvars(unusablePrimvarsMesh, options, channelPrimvars);
...
TEST(ChannelBuilderTests, TimeSampledPrimvar)
{
	...
	// Create a uniform interpolated primvar.
	pxr::UsdGeomPrimvarsAPI primVarApi(usdMesh.GetPrim());
	auto primvar = primVarApi.CreatePrimvar(pxr::TfToken("testPrimvar"), pxr::SdfValueTypeNames->Float3Array, pxr::UsdGeomTokens->uniform);

	auto setupAtTimeCode = [&](const pxr::UsdTimeCode& timeCode, const pxr::GfVec3f& value, int index)
	{
	...
	auto testAtTimeCode = [&](const pxr::UsdTimeCode& timeCode, const pxr::GfVec3f& expectedValue, int expectedIndex)
	{
...
TEST(ChannelBuilderTests, LeftHandedFaceOrientation)
{
	...
	auto vertexPrimvar = primVarApi.CreatePrimvar(pxr::TfToken(vertexPrimvarName), pxr::SdfValueTypeNames->Float3Array, pxr::UsdGeomTokens->vertex);
	vertexPrimvar.SetIndices({ 0,1,2,3 });
	vertexPrimvar.Set(pxr::VtVec3fArray{ pxr::GfVec3f(0, 1, 2), pxr::GfVec3f(3, 4, 5) , pxr::GfVec3f(6, 7, 8),pxr::GfVec3f(9, 10, 11) });
	...
	auto faceVaryingPrimvar = primVarApi.CreatePrimvar(pxr::TfToken(faceVaryingPrimvarName), pxr::SdfValueTypeNames->Float3Array, pxr::UsdGeomTokens->faceVarying);
		
	faceVaryingPrimvar.SetIndices({0,1,2,3});
	faceVaryingPrimvar.Set(pxr::VtVec3fArray{ pxr::GfVec3f(0, 1, 2), pxr::GfVec3f(3, 4, 5) , pxr::GfVec3f(6, 7, 8),pxr::GfVec3f(9, 10, 11) });
	...
	// 3) Test that map faces built from uniform interpolated primvars are not affected (all the points on a face
	// share the same data, nothing to flip).

	std::string uniformPrimvarName = "uniformPrimvar";
	// Create a unifrom interpolated primvar.
	auto uniformPrimvar = primVarApi.CreatePrimvar(pxr::TfToken(uniformPrimvarName), pxr::SdfValueTypeNames->Float3Array, pxr::UsdGeomTokens->uniform);
	...
	auto constantPrimvar = primVarApi.CreatePrimvar(pxr::TfToken(constantPrimvarName), pxr::SdfValueTypeNames->Float3Array, pxr::UsdGeomTokens->constant);
