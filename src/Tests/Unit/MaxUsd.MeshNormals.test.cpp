...
TEST(MeshNormalsTests, NormalsToConvert_None)
{
    ...
    converter.ConvertToUSDMesh(
        MaxUsd::MeshFacade { &cubeWithNormals },
        stage,
        path,
        options,
        usdMesh,
...
void UnspecifiedNormals_Test(bool withSmoothingGroups)
{
    ...
    MaxUsd::MeshConverter::ObjectChannelIntervals intervals;
    converter.ConvertToUSDMesh(
        MaxUsd::MeshFacade { &cubeWithUnspecifiedNormals },
        stage,
        path,
        options,
        usdMesh,
    ...
    converter.ConvertToMNMesh(
        usdMesh, reconvertedMesh, MaxUsd::PrimvarMappingOptions {}, channelNames);
...
TEST(MeshNormalsTests, UnspecifiedNormals_NoSmoothingGroups) { UnspecifiedNormals_Test(false); }

// Test that normals are converted if smoothing groups are defined, even if the
// normals are unspecified.
TEST(MeshNormalsTests, UnspecifiedNormals_WithSmoothingGroups) { UnspecifiedNormals_Test(true); }
...
void ConstantNormals_Test(bool asPrimvar)
{
    ...
    MaxUsd::MeshConverter            converter;
    MaxUsd::MaxMeshConversionOptions options;
    options.SetNormalsMode(
        asPrimvar ? MaxUsd::MaxMeshConversionOptions::NormalsMode::AsPrimvar
...
TEST(MeshNormalsTests, Constant_AsAttribute) { ConstantNormals_Test(false); }

// Test that the constant normal interpolation scheme is used when possible
// for normal primvars.
TEST(MeshNormalsTests, Constant_AsPrimvar) { ConstantNormals_Test(true); }
...
void Facevarying_Cube_Indexed(bool asPrimvar)
{
    ...
    if (asPrimvar) {
        auto normalsPrimvar = pxr::UsdGeomPrimvarsAPI(usdMesh.GetPrim())
                                  .GetPrimvar(pxr::UsdImagingTokens->primvarsNormals);
...
TEST(MeshNormalsTests, Facevarying_Cube_Indexed_AsPrimvar) { Facevarying_Cube_Indexed(true); }

// Test normal I/O with indexed max normals, converted as normals attributes.
// The normal indexing not expected to be preserved.
TEST(MeshNormalsTests, Facevarying_Cube_Indexed_AsAttibute) { Facevarying_Cube_Indexed(false); }
...
void FaceVarying_NotIndexed_Test(bool asPrimvar, bool ordered)
{
    ...
    // Not indexed so there will be duplications on purpose - we have numNormals = 2 faces * 4
    // vertices (quads).
    ...
    if (asPrimvar) {
        auto normalPrimvar = pxr::UsdGeomPrimvarsAPI(usdMesh.GetPrim())
                                 .GetPrimvar(pxr::UsdImagingTokens->primvarsNormals);
        normalsAttribute = normalPrimvar.GetAttr();

        // Expect face varying normal interpolation, because some the roof "top" vertices have
        // multiple normals, for each adjacent face.
        EXPECT_EQ(normalPrimvar.GetInterpolation(), pxr::UsdGeomTokens->faceVarying);

        // No need for an index, as the normals there are 8 normals for 8 vertex indices. Although
        // contrary to the ToUsdNormals_FaceVarying_Unordered_Indexed_AsPrimvar test, here some
        // remapping has to happen so that the normals are in order.
    ...
    if (ordered) {
        ...
        std::map<int, std::string> channelNames;
        converter.ConvertToMNMesh(
            usdMesh, reconverted, MaxUsd::PrimvarMappingOptions {}, channelNames);
...
void VertexNormals_Test(bool asPrimvar, bool indexed)
{
    ...
    if (asPrimvar) {
        normalPrimvar = pxr::UsdGeomPrimvarsAPI(usdMesh.GetPrim())
                            .GetPrimvar(pxr::UsdImagingTokens->primvarsNormals);
    ...
    const auto leftSideNormalUsd
        = pxr::GfVec3f(leftSideNormal.x, leftSideNormal.y, leftSideNormal.z);
    const auto topNormalUsd = pxr::GfVec3f(topNormal.x, topNormal.y, topNormal.z);
    ;
    const auto rightSideNormalUsd
        = pxr::GfVec3f(rightSideNormal.x, rightSideNormal.y, rightSideNormal.z);
...
TEST(MeshNormalsTests, Vertex_Indexed_AsPrimvar) { VertexNormals_Test(true, true); }

// Test normal I/O where normals should converted as attributes
// with vertex interpolation.
TEST(MeshNormalsTests, Vertex_Indexed_AsAttribute) { VertexNormals_Test(false, true); }

// Test normal I/O where normals should converted as primvar
// with vertex interpolation and no indexing.
TEST(MeshNormalsTests, Vertex_NotIndexed_AsPrimvar) { VertexNormals_Test(true, false); }

// Test normal I/O where normals should converted as attributes
// with vertex interpolation.
TEST(MeshNormalsTests, Vertex_NotIndexed_AsAttribute) { VertexNormals_Test(false, false); }
...
TEST(MeshNormalsTests, Uniform_ToMax_AsPrimvar) { UniformNormals_ToMax_Test(true); }

// Test import of uniform interpolation normals, when defined as attributes.
TEST(MeshNormalsTests, Uniform_ToMax_AsAttribute) { UniformNormals_ToMax_Test(false); }
...
TEST(MeshNormalsTests, Vertex_UnusedVertices_AsPrimvar) { VertexNormalsUnusedVertices_Test(true); }
...
// Test time sampled normals specified as attribute (animated Primvars are already tested in the
...
TEST(MeshNormalsTests, TimeSampledNormals_AsAttribute)
{
    ...
    const pxr::VtVec3fArray normalsDefault
        = { { 1.f, 1.f, 1.f }, { 2.f, 2.f, 2.f }, { 3.f, 3.f, 4.f } };
...
TEST(MeshNormalsTests, LeftHandedFaceOrientation)
{
    ...
    converter.ConvertToMNMesh(
        usdMesh, importedQuadVertex, MaxUsd::PrimvarMappingOptions {}, channelNames1);
    ...
    converter.ConvertToMNMesh(
        usdMesh, importedQuadFaceVaring, MaxUsd::PrimvarMappingOptions {}, channelNames2);
    ...
    // 3) Test that normal faces built from uniform interpolated primvars are not affected (all the
    ...
    converter.ConvertToMNMesh(
        usdMesh, importedQuadUniform, MaxUsd::PrimvarMappingOptions {}, channelNames3);
    ...
    converter.ConvertToMNMesh(
        usdMesh, importedQuadConstant, MaxUsd::PrimvarMappingOptions {}, channelNames4);
...
void ConvertNormalsToUsdTimeCode(bool asAttribute)
{
    ...
    std::map<MtlID, pxr::VtIntArray> materialIdToFacesMap;
    options.SetNormalsMode(
        asAttribute ? MaxUsd::MaxMeshConversionOptions::NormalsMode::AsAttribute
                    : MaxUsd::MaxMeshConversionOptions::NormalsMode::AsPrimvar);
    MaxUsd::MeshConverter::ObjectChannelIntervals intervals;
    // Default
    converter.ConvertToUSDMesh(
        MaxUsd::MeshFacade { &quad1 },
        stage,
        path,
        options,
        usdMesh,
    ...
    pxr::UsdAttribute normalsAttribute = asAttribute
        ...
        : pxr::UsdGeomPrimvarsAPI(usdMesh.GetPrim())
              .GetPrimvar(pxr::UsdImagingTokens->primvarsNormals)
              .GetAttr();

    pxr::VtVec3fArray normals1;
    normalsAttribute.Get(&normals1, pxr::UsdTimeCode::Default());
    const pxr::VtVec3fArray expectedNormals1
        = { { 1.f, 0.f, 0.f }, { 1.f, 1.f, 0.f }, { 1.f, 1.f, 1.f }, { 0.f, 1.f, 1.f } };
    EXPECT_EQ(expectedNormals1, normals1);

    pxr::VtVec3fArray normals2;
    normalsAttribute.Get(&normals2, pxr::UsdTimeCode(1));
    const pxr::VtVec3fArray expectedNormals2
        = { { 2.f, 0.f, 0.f }, { 2.f, 2.f, 0.f }, { 2.f, 2.f, 2.f }, { 0.f, 2.f, 2.f } };
    EXPECT_EQ(expectedNormals2, normals2);

    pxr::VtVec3fArray normals3;
    normalsAttribute.Get(&normals3, pxr::UsdTimeCode(2));
    const pxr::VtVec3fArray expectedNormals3
        = { { 3.f, 0.f, 0.f }, { 3.f, 3.f, 0.f }, { 3.f, 3.f, 3.f }, { 0.f, 3.f, 3.f } };
