...
TEST(ViewportInstancing, SceneGraphInstances)
{
    ...
    const auto expectedTransformBox001 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances/Box001")))
            .ComputeLocalToWorldTransform(0));
    EXPECT_TRUE(expectedTransformBox001.Equals(transforms1[0]));
    const auto expectedTransformBox003 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances/Box003")))
            .ComputeLocalToWorldTransform(0));
    ...
    const auto expectedTransformBox002 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances/Box002")))
            .ComputeLocalToWorldTransform(0));
    EXPECT_TRUE(expectedTransformBox002.Equals(transforms2[0]));
    const auto expectedTransformBox004 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances/Box004")))
            .ComputeLocalToWorldTransform(0));
...
TEST(ViewportInstancing, SceneGraphInstancesWithSubsets)
{
    ...
    // File contains a box instanced twice. Each of the faces has a different material. We therefor
...
#else
    const auto box1Path = pxr::SdfPath("/instances_with_material_bound_subsets/Box001.proto_Box001_id0");
#endif
    ...
    const auto expectedTransformBox001 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(
            stage->GetPrimAtPath(pxr::SdfPath("/instances_with_material_bound_subsets/Box001")))
            .ComputeLocalToWorldTransform(0));
    EXPECT_TRUE(expectedTransformBox001.Equals(transforms[0]));
    const auto expectedTransformBox002 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(
            stage->GetPrimAtPath(pxr::SdfPath("/instances_with_material_bound_subsets/Box002")))
            .ComputeLocalToWorldTransform(0));
...
#else
    const auto cube0Path
            = pxr::SdfPath("/Instancer.proto0_cube_id0");
#endif
...
TEST(ViewportInstancing, InstancesAnimatedTransform)
{
    ...
    auto expectedTransformBox001 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(
            stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances_animated/Box001")))
            .ComputeLocalToWorldTransform(0));
    EXPECT_TRUE(expectedTransformBox001.Equals(transforms1[0]));
    auto expectedTransformBox002 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(
            stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances_animated/Box002")))
            .ComputeLocalToWorldTransform(0));
    ...
    expectedTransformBox001 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(
            stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances_animated/Box001")))
            .ComputeLocalToWorldTransform(1));
    EXPECT_TRUE(expectedTransformBox001.Equals(transforms2[0]));
    expectedTransformBox002 = MaxUsd::ToMaxMatrix3(
        pxr::UsdGeomImageable(
            stage->GetPrimAtPath(pxr::SdfPath("/scene_graph_instances_animated/Box002")))
            .ComputeLocalToWorldTransform(1));
