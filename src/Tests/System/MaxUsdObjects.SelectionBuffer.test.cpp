...
// Utility function to test a render item's selection buffer state, fully selected or not (all ones or zeros)
void TestSelectionBufferState(
		const MaxSDK::Graphics::RenderItemHandle& renderItem, bool selected, bool decorated = true)
{
	...
	const auto selData = reinterpret_cast<Point3*>(selBuffer.Lock(0, 0, MaxSDK::Graphics::ReadAcess));
	...
	EXPECT_TRUE(std::all_of(selData, selData + selBuffer.GetNumberOfVertices(),
			[&expected](const Point3& p) { return p.Equals(expected); }));
...
TEST(SelectionBuffer, SelectDisplay)
{
	...
	TestRender(stageObject->GetUSDStage(), *(stageObject->GetHydraEngine()), renderItems, 0, nullptr,
	...
	auto ufeItem =
			Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, pxr::SdfPath("/root/Box001")));
...
TEST(SelectionBuffer, ConsolidatedMeshSelection)
{
	...
	stageObject->UpdatePrimSelectionDisplay();
	TestRender(stageObject->GetUSDStage(), *(stageObject->GetHydraEngine()), renderItems, 0, nullptr,
			{ pxr::HdReprTokens->smoothHull }, consolidationConfig);
	...
	Ufe::Selection newSelection;
	const auto ufeItem =
			Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, pxr::SdfPath("/root/Sphere001")));
	newSelection.append(ufeItem);
	globalUfeSel->replaceWith(newSelection);
	...
	EXPECT_TRUE(std::all_of(selData, selData + 24, [](const Point3& p) { return p.Equals(Point3{}); }));
	// The rest belong to the sphere, selected.
	EXPECT_TRUE(std::all_of(selData + 24, selData + selBuffer.GetNumberOfVertices(), [](const Point3& p) {
...
TEST(SelectionBuffer, ConsolidatedInstancedGeometry)
{
	...
	Ufe::Selection newSelection;
	const auto ufeItem =
			Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, pxr::SdfPath("/root/Box002")));
	newSelection.append(ufeItem);
	globalUfeSel->replaceWith(newSelection);
	stageObject->UpdatePrimSelectionDisplay();
	GetCOREInterface()->SelectNode(node);
	...
	EXPECT_TRUE(std::all_of(selData + 48, selData + selBuffer.GetNumberOfVertices(),
			[](const Point3& p) { return p.Equals(Point3{}); }));
...
TEST(SelectionBuffer, InstancedGeometry)
{
	...
	// Instance selection display is implemented using a different instance render item. So we now expect 2 items.
	...
	const auto ufeItemBBox003 =
			Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, pxr::SdfPath("/root/Box003")));
...
TEST(SelectionBuffer, SelectHierarchy)
{
	...
	const auto& globalUfeSel = Ufe::GlobalSelection::get();
	Ufe::Selection newSelection;
	const auto rootItem = Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, pxr::SdfPath("/root")));
...
TEST(SelectionBuffer, SelectedPrimAddRemove)
{
	...
	Ufe::Selection newSelection;
	newSelection.append(Ufe::Hierarchy::createItem(
			MaxUsd::ufe::getUsdPrimUfePath(stageObject, box3Path)));
	globalUfeSel->replaceWith(newSelection);
...
TEST(SelectionBuffer, InstancesIndexChange)
{
	...
	const auto& globalUfeSel = Ufe::GlobalSelection::get();
	Ufe::Selection newSelection;
	newSelection.append(Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, box1Path)));
	newSelection.append(Ufe::Hierarchy::createItem(MaxUsd::ufe::getUsdPrimUfePath(stageObject, box3Path)));
	globalUfeSel->replaceWith(newSelection);
	...
	EXPECT_TRUE(std::all_of(selData, selData + 24, [](const Point3& p) { return p.Equals(Point3{ 1.f, 1.f, 1.f }); }));
	...
	EXPECT_TRUE(std::all_of(selData + 48, selData + selBuffer.GetNumberOfVertices(),
			[](const Point3& p) { return p.Equals(Point3{ 1.f, 1.f, 1.f }); }));
	...
	EXPECT_TRUE(std::all_of(selData + 24, selData + selBuffer.GetNumberOfVertices(),
			[](const Point3& p) { return p.Equals(Point3{ 1.f, 1.f, 1.f }); }));
}