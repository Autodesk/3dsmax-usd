...
TEST(USDRenderDelegateGeneralTest, SimpleCube)
{
    ...
    std::array<int, 36> expectedIndices = { 0, 1, 2, 0, 2, 3, 4, 5, 6, 4, 6, 7, 0, 6, 5, 0, 5, 1,
    ...
    std::array<int, 48> wireframeExpectedIndices
        = { 0, 1, 1, 2, 2, 3, 3, 0, 4, 5, 5, 6, 6, 7, 7, 4, 0, 6, 6, 5, 5, 1, 1, 0,
            4, 7, 7, 3, 3, 2, 2, 4, 0, 3, 3, 7, 7, 6, 6, 0, 4, 2, 2, 1, 1, 5, 5, 4 };
...
// Tests rendering animated geometry to a nitrous render items. Points and topology change over
// time.
...
TEST(USDRenderDelegateGeneralTest, PrimTransforms)
{
    ...
    auto expectMatricesEqual
        = [](const MaxSDK::Graphics::Matrix44& m1, const MaxSDK::Graphics::Matrix44& m2) {
              for (int i = 0; i < 4; ++i) {
                  for (int j = 0; j < 4; ++j) {
...
TEST(USDRenderDelegateGeneralTest, PrimDisplayColorMaterial)
{
    ...
    const auto& material1 = static_cast<const MaxSDK::Graphics::StandardMaterialHandle&>(
        renderItem.GetCustomMaterial());
    ...
    const auto& material2 = static_cast<const MaxSDK::Graphics::StandardMaterialHandle&>(
        renderItem2.GetCustomMaterial());
