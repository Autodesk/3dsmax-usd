...
{
public:
	MockUpdateNodeContext()
	{
		this->GetRenderNode().Initialize();
	}	
};
...
{
public:
	size_t GetNumberOfRenderItems() const override
	{
	...
	const MaxSDK::Graphics::RenderItemHandleDecorator& At(size_t i)
	{
		return renderItems[i];
	}
	...
	{
		const auto renderItem = dynamic_cast<const MaxSDK::Graphics::RenderItemHandleDecorator&>(pRenderItem);
	...
	void RemoveRenderItem(size_t i) override
	{
		renderItems.erase(renderItems.begin() + i);
	}

	void ClearAllRenderItems() override
	{
		renderItems.clear();
	}
...
void TestRender(pxr::UsdStageRefPtr stage, HdMaxEngine& engine, MaxSDK::Graphics::IRenderItemContainer& renderItems,
		const pxr::UsdTimeCode& timeCode, MultiMtl* multiMat = nullptr, const pxr::TfTokenVector& reprs = {pxr::HdReprTokens->smoothHull}, const HdMaxConsolidator::Config& consolidationConfig = {});

/**
 * \brief Helper to fetch the render geometry from a render item. Used to validate render results.
 * Caller must specify if the render item is decorated (has an offset transform), and if it is a render
 * item also displaying selection (this uses a Custom render item internally)
...
MaxSDK::Graphics::SimpleRenderGeometry* GetRenderItemGeometry(
		const MaxSDK::Graphics::RenderItemHandle& renderItem, bool decorated = true, bool selectionRenderItem = false);
