...
class TestReader(maxUsd.PrimReader):
    ...
    def CanImport(cls, args, prim):
        ...
        try: 
            ...
            rt.assert_equal(2, len(options.GetShadingModes()))
            rt.assert_equal("useRegistry", options.GetShadingModes()[1]['mode'])
            rt.assert_equal("UsdPreviewSurface", options.GetShadingModes()[1]['materialConversion'])
            rt.assert_equal("useRegistry", options.GetShadingModes()[0]['mode'])
            rt.assert_equal("MaterialXReference", options.GetShadingModes()[0]['materialConversion'])
