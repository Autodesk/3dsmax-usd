    ...
    if (consolidation) {
        for (const auto& consolidatedGeom : *consolidation->geoms) {
            ...
            if (needWireRepr || isGizmo) {
                ...
                if (isGizmo) {
                    const auto mtl = HdMaxMaterialCollection::GetGizmoMaterial(
                        nodeContext.GetRenderNode(), consolidatedGeom->hasActiveSelection);
    ...
    for (const auto& primRenderData : renderData) {
        ...
        if (needWireRepr || primRenderData->isGizmo) {
            ...
            if (!primRenderData->wireframe.IsInstanced()) {
                ...
                // When displaying selection highlighting, use a custom render item able to render
                // everything : the geometry AND the highlighting. Because of limitations with how
                // gizmos are drawn, gizmos instead rely on different materials to display
                // selection.
                bool  useSelectionRenderItem = primRenderData->selected && !primRenderData->isGizmo;
                auto& wireRenderItem
                ...
                if (primRenderData->isGizmo) {
                    wireRenderItem.SetVisibilityGroup(MaxSDK::Graphics::RenderItemVisible_Gizmo);

                    const auto mtl = HdMaxMaterialCollection::GetGizmoMaterial(
                        nodeContext.GetRenderNode(), primRenderData->selected);
...
bool HdMaxEngine::UpdateRootPrim(const pxr::UsdPrim& rootPrim, Mtl* nodeMaterial)
{
    ...
    if (auto mergingSceneIndex = MaxUsd::FindTopLevelMergingSceneIndex(terminalIndex)) {
        ...
        sceneDelegate->GetRenderIndex().InsertSceneIndex(filter, pxr::SdfPath { "/" });
        filter->NotifyInitialAddedPrims();
    }
#endif
