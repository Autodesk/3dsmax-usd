...
*/
HdMaxBufferSampler::HdMaxBufferSampler(HdVtBufferSource const& buffer)
    : _buffer(buffer)
{
}
...
*/
bool HdMaxBufferSampler::Sample(int index, void* value, HdTupleType dataType) const
{
    ...
    memcpy_s(value, elemSize, static_cast<const uint8_t*>(_buffer.GetData()) + offset, elemSize);
    return true;
}
...
*/
bool HdMaxPrimvarSampler::_Interpolate(
    void*       out,
    void**      samples,
    float*      weights,
    size_t      sampleCount,
    HdTupleType dataType)
{
    ...
    short numComponents = short(HdGetComponentCount(dataType.type) * dataType.count);
