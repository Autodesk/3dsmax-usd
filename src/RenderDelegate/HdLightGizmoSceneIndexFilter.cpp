    ...
    HdSceneIndexObserver::AddedPrimEntries& addedPrims)
{
    ...
    addedPrim.primPath = GetLightSPrimPath(primPath);
...
pxr::SdfPath HdLightGizmoSceneIndexFilter::GetLightSPrimPath(const pxr::SdfPath& path) const
{
    if (path.IsPrimPath()) {
        return path.AppendChild(lightName);
    }
    // Instanced light prototype path.
    return pxr::SdfPath { path.GetString() + std::string("_") + lightName.GetString() };
}
...
std::vector<pxr::SdfPath> HdLightGizmoSceneIndexFilter::GetHandledLights() const
{
    std::vector<pxr::SdfPath> lightPaths;
    lightPaths.reserve(lightGizmos.size());
    for (const auto& entry : lightGizmos) {
        lightPaths.push_back(entry.first);
    }
    return lightPaths;
}
    ...
    const HdSceneIndexObserver::RemovedPrimEntries& entries)
{
    ...
    for (const auto& entry : entries) {
        if (lightGizmos.find(entry.primPath) != lightGizmos.end()) {
            const auto lightSPrimPath = GetLightSPrimPath(entry.primPath);
            removedEntries.push_back(lightSPrimPath);
    ...
    const HdSceneIndexObserver::DirtiedPrimEntries& entries)
{
    ...
    for (const auto& entry : entries) {
        if (lightGizmos.find(entry.primPath) != lightGizmos.end()) {
            ...
            lightChildEntry.primPath = GetLightSPrimPath(entry.primPath);
