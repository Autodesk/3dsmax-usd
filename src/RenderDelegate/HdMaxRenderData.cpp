    ...
    bool                                      instanced) const
{
    if (renderData.isGizmo) {
        return HdMaxMaterialCollection::GetGizmoMaterial(renderNode, renderData.selected);
    }
    ...
    case HdMaxDisplaySettings::USDPreviewSurface: {
        const SubsetRenderData&              subsetGeometry = renderData.shadedSubsets[subsetIndex];
