...
// Define template specializations of HdMaxTypeHelper methods for
// all our supported types...
#define TYPE_HELPER(T, type)                                          \
    template <> inline HdTupleType HdMaxTypeHelper::GetTupleType<T>() \
    {                                                                 \
...
{
public:
    HdMaxBufferSampler(HdVtBufferSource const& buffer);
    ...
    {
        return Sample(index, static_cast<void*>(value), HdMaxTypeHelper::GetTupleType<T>());
    }
...
public:
    //! Default constructor.
    HdMaxPrimvarSampler() = default;
    //! Default destructor.
    virtual ~HdMaxPrimvarSampler() = default;
    ...
    */
    virtual bool
    Sample(unsigned int element, float u, float v, void* value, HdTupleType dataType) const
        = 0;

    //! Convenient, templated frontend for Sample().
    template <typename T> bool Sample(unsigned int element, float u, float v, T* value) const
    {
        return Sample(element, u, v, static_cast<void*>(value), HdMaxTypeHelper::GetTupleType<T>());
    }
