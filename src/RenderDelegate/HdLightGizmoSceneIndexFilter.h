...
public:
    ...
    /**
     * Returns the paths of all lights currently handled by the filter.
     * @return The light paths.
     */
    std::vector<SdfPath> RenderDelegateAPI GetHandledLights() const;
...
private:
    ...
    /**
     * The filter replaces lights with gizmo geometry, and reparents the lights themselves
     * under the gizmo. This method returns the path of that reparented light SPrim.
     * For instanced lights, the SPrim path is created by appending to the gizmo proto path.
     * @param path The source light path.
     * @return The new light SPrim path
     */
    pxr::SdfPath GetLightSPrimPath(const pxr::SdfPath& path) const;
