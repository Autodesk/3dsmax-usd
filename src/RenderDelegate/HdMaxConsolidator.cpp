    ...
    for (const auto& materialCells : cells) {
        for (const auto& cell : materialCells.second) {
            ...
            // Build wireframe render item for the consolidated cell.
            {
                ...
                bool isGizmo = result->material
                        == HdMaxMaterialCollection::GetGizmoMaterial(renderNode, false)
                    || result->material
                        == HdMaxMaterialCollection::GetGizmoMaterial(renderNode, true);
                ...
                result->wireframeRenderItemSelection.SetVisibilityGroup(
                    !isGizmo ? MaxSDK::Graphics::RenderItemVisible_Wireframe
                             : MaxSDK::Graphics::RenderItemVisible_Gizmo);
                result->wireframeRenderItemSelection.SetCustomMaterial(
                    renderNode.GetWireframeMaterial());
            }
    ...
    std::vector<std::pair<MaxSDK::Graphics::BaseMaterialHandle, size_t>>& materialTris) const
{
    ...
    for (int i = 0; i < renderData.shadedSubsets.size(); ++i) {
        subsetInfos[i].material = renderData.ResolveViewportMaterial(
            renderData, i, config.displaySettings, renderNode, false);
    ...
    const pxr::UsdTimeCode&                   newTime)
{
    ...
    for (const auto& primData : renderData) {
        ...
        for (int i = 0; i < primData->shadedSubsets.size(); ++i) {
            ...
            // meshes so re-consolidating is quite fast.
            bool gizmoSelectionDirty
                = primData->isGizmo
                && HdMaxChangeTracker::CheckDirty(
                      data.dirtyBits, HdMaxChangeTracker::DirtySelectionHighlight);
            breakConsolidation = breakConsolidation || gizmoSelectionDirty;
            ...
            for (int i = 0; i < primData.consolidatedSubsets.size(); ++i) {
                auto material = primData.primRenderData->ResolveViewportMaterial(
                    *primData.primRenderData,
                    primData.consolidatedSubsets[i],
