...
TF_DECLARE_REF_PTRS(MaxUsdExportChaser);

/// \brief base class for plugin chasers which are plugins that run after the
/// core usdExport out of 3ds Max.
///
/// Chaser objects will be constructed after the initial export.
...
/// small changes or to add attributes, in a non-destructive way, to an exported stage.
class MaxUsdExportChaser : public TfRefBase
{
public:
    ~MaxUsdExportChaser() override { }
